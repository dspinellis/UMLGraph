...
class ClassGraph {
    ...
    /**
     * Return as a string the stereotypes associated with c terminated by the escape
     * character term
     */
    ...
    private boolean hidden(ProgramElementDoc c) {
        ...
        return opt.matchesHideExpression(c.toString()) //
                || (opt.hidePrivateInner && c instanceof ClassDoc && c.isPrivate()
                        && ((ClassDoc) c).containingClass() != null);
    }
    ...
    public String printClass(ClassDoc c, boolean rootClass) {
        ...
        boolean showMembers = (opt.showAttributes && c.fields().length > 0)
        ...
        for (Tag tag : from.tags(tagname)) {
            ...
            if (t.length != 4) {
                System.err.println("Error in " + from + "\n" + tagname
                        + " expects four fields (l-src label l-dst target): " + tag.text());
                return;
            }
     ...
     */
    private void relation(Options opt, RelationType rt, ClassDoc from, String fromName, ClassDoc to, String toName,
     ...
     */
    private void relation(Options opt, RelationType rt, ClassDoc from, ClassDoc to, String tailLabel, String label,
            String headLabel) {
    ...
    public String classToUrl(ClassDoc cd, boolean rootClass) {
        ...
        if (contextPackageName != null && rootClass)
            return buildRelativePathFromClassNames(contextPackageName, cd.containingPackage().name()) + cd.name()
                    + ".html";
